...
def test_policy_text_presence_violation(tmp_path: Path) -> None:
...
def test_descriptor_drift_emits_warning(tmp_path: Path) -> None:
...
def test_registry_mismatch_raises_error(tmp_path: Path) -> None:
...
def test_status_update_required_when_watched_files_change(
    tmp_path: Path,
) -> None:
...
def test_status_payload_validation_passes(tmp_path: Path) -> None:
