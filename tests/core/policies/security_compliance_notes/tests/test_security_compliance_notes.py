...
from devcovenant.core.policies.security_compliance_notes import (
    security_compliance_notes,
)


def _checker() -> security_compliance_notes.SecurityComplianceNotesCheck:
    """Return the policy configured for guarded paths."""
    checker = security_compliance_notes.SecurityComplianceNotesCheck()
    checker.set_options(
        {
