...
MODULE = "devcovenant.core.translator"
            ...
            "translators": [
                {
                    ...
                    "can_handle": {
                        ...
                        "entrypoint": "devcovenant.core."
                        "translator.can_handle",
                    },
                    "translate": {
                        "strategy": "module_function",
                        "entrypoint": "devcovenant.core."
                        "translator.translate_language_unit",
                    },
                }
            ],
        }
...
def _unit_test_translate_sanitizes_language_unit_path() -> None:
    """Translated units should expose repo-safe paths, not absolute ones."""
    module = importlib.import_module(MODULE)
    with tempfile.TemporaryDirectory() as tmpdir:
        repo_root = Path(tmpdir)
        path = repo_root / "nested" / "sample.py"
        ...
        resolution = module.TranslatorResolution(declaration, tuple())
        source = "print('x')\n"
        absolute_unit = module.LanguageUnit(
            translator_id="python",
            profile_name="python",
            language="python",
        ...
        assert translated.path == "nested/sample.py"
        assert str(repo_root) not in translated.path
...
class TranslatorTests(unittest.TestCase):
    ...
    def test_translate_sanitizes_language_unit_path(self):
        """Run translator path-sanitization assertions."""
        _unit_test_translate_sanitizes_language_unit_path()
