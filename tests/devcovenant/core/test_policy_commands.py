"""Tests for devcovenant.core.policy_commands."""
...
def _policy_commands_seed_registry(repo_root: Path) -> None:
...
def _policy_cmd_find_policy_command_resolves_declared_name() -> None:
    """Command lookup should resolve the declared canonical name."""
    with tempfile.TemporaryDirectory() as temp_dir:
        repo_root = Path(temp_dir).resolve()
        _policy_commands_seed_registry(repo_root)
...
def _policy_cmd_parse_policy_command_payload_uses_declared_arguments() -> None:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        payload = policy_commands.parse_policy_command_payload(
            "dependency-management", command, ["--scope", "locks"]
        )
        assert payload == {"scope": "locks"}


class PolicyCommandsTests(unittest.TestCase):
    ...
    def test_find_policy_command_resolves_declared_name(self):
        ...
        _policy_cmd_find_policy_command_resolves_declared_name()

    def test_parse_policy_command_payload_uses_declared_arguments(self):
        """Run argument parsing assertions."""
        _policy_cmd_parse_policy_command_payload_uses_declared_arguments()
