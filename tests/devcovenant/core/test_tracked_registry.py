"""Tests for devcovenant.core.tracked_registry."""
...
MODULE = "devcovenant.core.tracked_registry"


def _tracked_module_importable() -> None:
    """Tracked registry module should import successfully."""
    module = importlib.import_module(MODULE)
    assert module is not None


def _tracked_public_symbol_contract_is_stable() -> None:
...
def _tracked_path_helpers_resolve_tracked_locations() -> None:
    """Tracked registry helpers should resolve tracked paths only."""
    module = importlib.import_module(MODULE)
    with tempfile.TemporaryDirectory() as tmpdir:
        repo_root = Path(tmpdir)
        assert (
            module.registry_root(repo_root)
            == repo_root / "devcovenant" / "registry"
        )
        assert (
            module.policy_registry_path(repo_root)
            == repo_root / "devcovenant" / "registry" / "registry.yaml"
        )
        assert (
            module.profile_registry_path(repo_root)
            == repo_root / "devcovenant" / "registry" / "registry.yaml"
        )


def _tracked_document_helpers_round_trip_registry_payload() -> None:
