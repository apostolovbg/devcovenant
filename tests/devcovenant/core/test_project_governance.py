"""Tests for devcovenant.core.project_governance."""
...
def _project_governance_write_project_governance_config(
    repo_root: Path, **updates: object
) -> None:
...
class ProjectGovernanceTests(unittest.TestCase):
    ...
    def test_runtime_state_defaults_to_unversioned_governance(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(state.copyright_notice, "YEAR Legal Owner Name")
    ...
    def test_service_registry_payload_is_deterministic(self):
        ...
        self.assertEqual(
            payload["copyright_notice"], "2026 DevCovenant Authors"
        )
        ...
        self.assertEqual(payload["compatibility_policy"], "forward-only")
    ...
    def test_section_lines_surface_compatibility_guidance(self):
        ...
        self.assertIn("- Compatibility Guidance:", lines)
        self.assertIn(
            (
                "  Do not leave legacy fallbacks behind. Remove "
                "deprecated readers,"
            ),
            lines,
        )
        self.assertIn(
            "  aliases, and bridge paths instead of preserving them.", lines
        )
    ...
    def test_render_identity_placeholders_wraps_project_description(self):
        ...
        rendered = render_identity_placeholders(
            "{{ PROJECT_DESCRIPTION_PARAGRAPH }}", state
        )
        self.assertIn("\n", rendered)
        self.assertEqual(" ".join(rendered.split()), state.project_description)
    ...
    def test_render_identity_placeholders_formats_toml_description(self):
        ...
        self.assertEqual(
            tomllib.loads(rendered)["description"], state.project_description
        )
    ...
    def test_render_identity_placeholders_replaces_copyright_notice(self):
        ...
        rendered = render_identity_placeholders(
            "Copyright (c) {{ COPYRIGHT_NOTICE }}", state
        )
