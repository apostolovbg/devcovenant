...
def _unit_test_requires_tests_for_code_change(tmp_path: Path) -> None:
...
def _unit_test_start_phase_skips_missing_status(
...
def _unit_test_passes_when_tests_are_fresh(tmp_path: Path) -> None:
...
def _unit_test_start_after_edit_is_warning(tmp_path: Path) -> None:
...
def _unit_test_requires_pre_commit_start(tmp_path: Path) -> None:
...
def _unit_test_requires_pre_commit_end(tmp_path: Path) -> None:
...
def _unit_test_any_change_requires_tests(tmp_path: Path) -> None:
...
def _unit_test_custom_status_path(tmp_path: Path) -> None:
...
def _unit_test_metadata_config_overrides(tmp_path: Path) -> None:
...
class GeneratedUnittestCases(unittest.TestCase):
    ...
    def test_requires_tests_for_code_change(self):
        """Run test_requires_tests_for_code_change."""
        with tempfile.TemporaryDirectory() as temp_dir:
            tmp_path = Path(temp_dir).resolve()
            _unit_test_requires_tests_for_code_change(tmp_path=tmp_path)
    ...
    def test_start_phase_skips_missing_status(self):
        ...
        try:
            with tempfile.TemporaryDirectory() as temp_dir:
                tmp_path = Path(temp_dir).resolve()
                _unit_test_start_phase_skips_missing_status(
                    tmp_path=tmp_path, monkeypatch=monkeypatch
                )
        finally:
            monkeypatch.undo()

    def test_passes_when_tests_are_fresh(self):
        """Run test_passes_when_tests_are_fresh."""
        with tempfile.TemporaryDirectory() as temp_dir:
            tmp_path = Path(temp_dir).resolve()
            _unit_test_passes_when_tests_are_fresh(tmp_path=tmp_path)

    def test_start_after_edit_is_warning(self):
        """Run test_start_after_edit_is_warning."""
        with tempfile.TemporaryDirectory() as temp_dir:
            tmp_path = Path(temp_dir).resolve()
            _unit_test_start_after_edit_is_warning(tmp_path=tmp_path)

    def test_requires_pre_commit_start(self):
        """Run test_requires_pre_commit_start."""
        with tempfile.TemporaryDirectory() as temp_dir:
            tmp_path = Path(temp_dir).resolve()
            _unit_test_requires_pre_commit_start(tmp_path=tmp_path)

    def test_requires_pre_commit_end(self):
        """Run test_requires_pre_commit_end."""
        with tempfile.TemporaryDirectory() as temp_dir:
            tmp_path = Path(temp_dir).resolve()
            _unit_test_requires_pre_commit_end(tmp_path=tmp_path)

    def test_any_change_requires_tests(self):
        """Run test_any_change_requires_tests."""
        with tempfile.TemporaryDirectory() as temp_dir:
            tmp_path = Path(temp_dir).resolve()
            _unit_test_any_change_requires_tests(tmp_path=tmp_path)

    def test_custom_status_path(self):
        """Run test_custom_status_path."""
        with tempfile.TemporaryDirectory() as temp_dir:
            tmp_path = Path(temp_dir).resolve()
            _unit_test_custom_status_path(tmp_path=tmp_path)

    def test_metadata_config_overrides(self):
        """Run test_metadata_config_overrides."""
        with tempfile.TemporaryDirectory() as temp_dir:
            tmp_path = Path(temp_dir).resolve()
            _unit_test_metadata_config_overrides(tmp_path=tmp_path)
