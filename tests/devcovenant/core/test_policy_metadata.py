"""Tests for devcovenant.core.policy_metadata."""
...
def _metadata_module_importable() -> None:
    """Module should import cleanly."""
    module = importlib.import_module(MODULE)
    assert module is not None


def _metadata_module_has_public_symbols() -> None:
...
def _metadata_metadata_symbol_contract_is_stable() -> None:
...
def _metadata_apply_policy_control_preserves_critical_enablement() -> None:
...
def _metadata_apply_policy_control_allows_noncritical_disablement() -> None:
...
def _metadata_decode_metadata_option_value_normalizes_common_shapes() -> None:
    ...
    assert module.decode_metadata_option_value("a, b, c") == ["a", "b", "c"]
...
def _policy_metadata_empty_context(module):
...
def _metadata_resolved_bundle_preserves_string_map_contract() -> None:
    ...
    bundle = module.resolve_policy_metadata_bundle(
        ...
        {},
        descriptor,
        _policy_metadata_empty_context(module),
    )
...
def _metadata_resolved_bundle_tracks_layer_trace_and_warnings() -> None:
    ...
    bundle = module.resolve_policy_metadata_bundle(
        "demo-policy", [], {}, descriptor, context
    )
...
def _metadata_bundle_tracks_runtime_defaults_and_selector_derives() -> None:
    """Bundles should trace runtime defaults and selector-derived fields."""
    module = importlib.import_module(MODULE)
    descriptor = PolicyDescriptor(
        policy_id="demo-policy",
        text="demo",
        metadata={"watch_prefixes": ["docs"]},
    )
    bundle = module.resolve_policy_metadata_bundle(
        "demo-policy",
        [],
...
def _metadata_profile_overlays_collect_policy_sections() -> None:
...
def _metadata_profile_overlays_preserve_structured_mapping_lists() -> None:
...
class PolicyMetadataTests(unittest.TestCase):
    ...
    def test_resolve_policy_metadata_bundle_preserves_string_map_contract(
        ...
        _metadata_resolved_bundle_preserves_string_map_contract()
    ...
    def test_resolve_policy_metadata_bundle_tracks_layer_trace_and_warnings(
        ...
        _metadata_resolved_bundle_tracks_layer_trace_and_warnings()
...
def _parse_module_importable() -> None:
    """Module should import cleanly."""
    module = importlib.import_module(MODULE)
    assert module is not None


def _parse_module_has_public_symbols() -> None:
    """Module should expose at least one public symbol."""
    module = importlib.import_module(MODULE)
    public_symbols = [name for name in dir(module) if not name.startswith("_")]
    assert public_symbols
...
def _parse_symbol_contract_is_stable() -> None:
...
def _parse_parse_metadata_rejects_continuation_shortcuts() -> None:
    """Metadata parser should reject unsupported continuation shortcuts."""
    block = (
        "\n"
        "id: demo-policy\n"
        "severity: warning\n"
    ...
    try:
        parse_policy_metadata_block(block.strip())
    except ValueError as exc:
        assert "Invalid policy metadata YAML" in str(exc)
    else:
        raise AssertionError(
            "Expected continuation shortcut syntax to be rejected."
        )


class PolicyMetadataParseTests(unittest.TestCase):
    """unittest wrappers for layered module sanity checks."""

    def test_module_importable(self):
        """Run module importability sanity check."""
        _parse_module_importable()

    def test_module_has_public_symbols(self):
        """Run module public-symbol sanity check."""
        _parse_module_has_public_symbols()

    def test_symbol_contract_is_stable(self):
        """Run stable policy-parse symbol contract assertions."""
        _parse_symbol_contract_is_stable()

    def test_parse_metadata_rejects_continuation_shortcuts(self):
        """Run unsupported-metadata continuation rejection assertions."""
        _parse_parse_metadata_rejects_continuation_shortcuts()
