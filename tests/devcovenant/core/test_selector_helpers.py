...
def _unit_test_selector_matches_with_suffix_includes(tmp_path):
...
def _unit_test_selector_excludes_take_precedence(tmp_path):
...
def _unit_test_force_include_globs_override_excludes(tmp_path):
...
def _unit_test_selector_without_includes_matches_everything(tmp_path):
...
def _unit_test_build_watchlists_normalizes_paths():
...
def _unit_test_selector_prefixes_handle_relative_and_absolute(tmp_path):
...
class GeneratedUnittestCases(unittest.TestCase):
    ...
    def test_selector_matches_with_suffix_includes(self):
        """Run test_selector_matches_with_suffix_includes."""
        with tempfile.TemporaryDirectory() as temp_dir:
            tmp_path = Path(temp_dir).resolve()
            _unit_test_selector_matches_with_suffix_includes(tmp_path=tmp_path)

    def test_selector_excludes_take_precedence(self):
        """Run test_selector_excludes_take_precedence."""
        with tempfile.TemporaryDirectory() as temp_dir:
            tmp_path = Path(temp_dir).resolve()
            _unit_test_selector_excludes_take_precedence(tmp_path=tmp_path)

    def test_force_include_globs_override_excludes(self):
        """Run test_force_include_globs_override_excludes."""
        with tempfile.TemporaryDirectory() as temp_dir:
            tmp_path = Path(temp_dir).resolve()
            _unit_test_force_include_globs_override_excludes(tmp_path=tmp_path)

    def test_selector_without_includes_matches_everything(self):
        """Run test_selector_without_includes_matches_everything."""
        with tempfile.TemporaryDirectory() as temp_dir:
            tmp_path = Path(temp_dir).resolve()
            _unit_test_selector_without_includes_matches_everything(
                tmp_path=tmp_path
            )

    def test_build_watchlists_normalizes_paths(self):
        """Run test_build_watchlists_normalizes_paths."""
        _unit_test_build_watchlists_normalizes_paths()

    def test_selector_prefixes_handle_relative_and_absolute(self):
        """Run test_selector_prefixes_handle_relative_and_absolute."""
        with tempfile.TemporaryDirectory() as temp_dir:
            tmp_path = Path(temp_dir).resolve()
            _unit_test_selector_prefixes_handle_relative_and_absolute(
                tmp_path=tmp_path
            )
