...
def _unit_test_refresh_all_syncs_configured_autogen_docs_only(
    tmp_path: Path,
) -> None:
...
class GeneratedUnittestCases(unittest.TestCase):
    ...
    def test_refresh_all_syncs_configured_autogen_docs_only(self):
        """Run test_refresh_all_syncs_configured_autogen_docs_only."""
        with tempfile.TemporaryDirectory() as temp_dir:
            tmp_path = Path(temp_dir).resolve()
            _unit_test_refresh_all_syncs_configured_autogen_docs_only(
                tmp_path=tmp_path
            )
