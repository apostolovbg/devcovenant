...
def _unit_test_refresh_policies_uses_registry_as_source(
    tmp_path: Path,
) -> None:
...
def _unit_test_refresh_policies_no_registry_noop(tmp_path: Path) -> None:
...
class GeneratedUnittestCases(unittest.TestCase):
    ...
    def test_refresh_policies_uses_registry_as_source(self):
        """Run test_refresh_policies_uses_registry_as_source."""
        with tempfile.TemporaryDirectory() as temp_dir:
            tmp_path = Path(temp_dir).resolve()
            _unit_test_refresh_policies_uses_registry_as_source(
                tmp_path=tmp_path
            )

    def test_refresh_policies_no_registry_noop(self):
        """Run test_refresh_policies_no_registry_noop."""
        with tempfile.TemporaryDirectory() as temp_dir:
            tmp_path = Path(temp_dir).resolve()
            _unit_test_refresh_policies_no_registry_noop(tmp_path=tmp_path)
