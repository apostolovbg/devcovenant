...
def _unit_test_status_update_required_when_watched_files_change(
    ...
    assert any(
        "fresh gate status update" in item.message for item in violations
    )
...
def _unit_test_list_path_options_are_accepted(tmp_path: Path) -> None:
    """List-valued invariant path metadata should normalize safely."""
    agents_path = tmp_path / "AGENTS.md"
    _write_agents(agents_path, description="Policy description.")
    _write_policy_script(tmp_path)
    _write_descriptor(tmp_path, text_value="Policy description.")
...
class GeneratedUnittestCases(unittest.TestCase):
    ...
    def test_list_path_options_are_accepted(self):
        """Run list-path metadata normalization assertions."""
        with tempfile.TemporaryDirectory() as temp_dir:
            tmp_path = Path(temp_dir).resolve()
            _unit_test_list_path_options_are_accepted(tmp_path=tmp_path)
