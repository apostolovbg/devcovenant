"""Sanity and behavior checks for document exemption fingerprint helpers."""
...
MODULE = "devcovenant.core.lib.document_exemptions"
...
def _unit_test_document_exemptions_symbol_contract_is_stable() -> None:
...
def _unit_test_doc_header_and_managed_changes_keep_non_exempt_hash() -> None:
    ...
    assert module.managed_marker_signature(old_doc) == (
        module.managed_marker_signature(exempt_only_doc)
    )
    assert module.non_exempt_content_hash(old_doc, "notes.md", **kwargs) == (
...
def _unit_test_fingerprint_for_path_supports_yml_and_yaml_assets() -> None:
...
def _unit_test_normalize_document_exemption_entry_validates_shape() -> None:
    """
