...
MODULE = "devcovenant.core.cleanup"


def _cleanup_clean_flow_module_symbol_contract_is_stable() -> None:
    """Clean flow module should expose its orchestration entrypoint."""
    module = importlib.import_module(MODULE)
    assert module.clean_repo


def _cleanup_clean_flow_reports_scope_and_removed_targets() -> None:
    """Clean flow should report selection scope and removal counts."""
    module = importlib.import_module(MODULE)
    selection = module.CleanSelection(
        include_build=True,
        include_cache=False,
        include_runtime_registry=False,
        include_logs=False,
    )
    result = module.CleanResult(
        selection=selection,
        removed_paths=("build",),
    ...
    with patch.object(
        module, "resolve_clean_selection", return_value=selection
    ) as resolve_mock:
        with patch.object(
            module, "execute_cleanup", return_value=result
        ) as execute_mock:
            with (
                patch.object(
                    module, "_gate_session_is_open", return_value=False
                ),
...
def _cleanup_clean_flow_rejects_open_gate_session() -> None:
...
def _cleanup_clean_flow_protects_active_run_log_directory() -> None:
    ...
    execute_mock.assert_called_once_with(
        Path("."), selection, extra_protected_paths=(active_run_dir,)
    )


class CleanupCasesCommandTests(unittest.TestCase):
