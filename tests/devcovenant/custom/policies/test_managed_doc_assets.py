...
def _unit_test_managed_doc_assets_passes_when_descriptors_match(
    tmp_path: Path,
) -> None:
...
def _unit_test_managed_doc_assets_detects_descriptor_drift(
    tmp_path: Path,
) -> None:
...
class GeneratedUnittestCases(unittest.TestCase):
    ...
    def test_managed_doc_assets_passes_when_descriptors_match(self):
        """Run test_managed_doc_assets_passes_when_descriptors_match."""
        with tempfile.TemporaryDirectory() as temp_dir:
            tmp_path = Path(temp_dir).resolve()
            _unit_test_managed_doc_assets_passes_when_descriptors_match(
                tmp_path=tmp_path
            )

    def test_managed_doc_assets_detects_descriptor_drift(self):
        """Run test_managed_doc_assets_detects_descriptor_drift."""
        with tempfile.TemporaryDirectory() as temp_dir:
            tmp_path = Path(temp_dir).resolve()
            _unit_test_managed_doc_assets_detects_descriptor_drift(
                tmp_path=tmp_path
            )
