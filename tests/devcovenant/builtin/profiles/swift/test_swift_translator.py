"""Unit tests for the Swift language translator."""
...
_TRANSLATOR_PATH = (
    _REPO_ROOT / "devcovenant/builtin/profiles/swift/swift_translator.py"
)
...
def _load_translator_module():
    ...
    spec = importlib.util.spec_from_file_location(
        "swift_translator", _TRANSLATOR_PATH
    )
...
def _declaration() -> TranslatorDeclaration:
    ...
    return TranslatorDeclaration(
        ...
        can_handle_entrypoint="swift_translator.py:can_handle",
        translate_strategy="module_function",
        translate_entrypoint="swift_translator.py:translate",
    )


class SwiftTranslatorTests(unittest.TestCase):
    """Verify Swift translation behavior."""
    ...
    def test_can_handle_suffix(self):
        ...
        self.assertTrue(
            module.can_handle(
                path=Path("sample.swift"), declaration=_declaration()
            )
        )
    ...
    def test_translate_emits_identifier_and_risk_facts(self):
        ...
        unit = module.translate(
            path=Path("sample.swift"),
            source=(
                "// docs\nclass Runner {}\n"
                "func run() { let _ = Process() }\n"
            ),
