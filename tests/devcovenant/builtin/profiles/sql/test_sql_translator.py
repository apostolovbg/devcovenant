"""Unit tests for the SQL language translator."""
...
_TRANSLATOR_PATH = (
    _REPO_ROOT / "devcovenant/builtin/profiles/sql/sql_translator.py"
)
...
def _load_translator_module():
    ...
    spec = importlib.util.spec_from_file_location(
        "sql_translator", _TRANSLATOR_PATH
    )
...
def _declaration() -> TranslatorDeclaration:
    ...
    return TranslatorDeclaration(
        ...
        can_handle_entrypoint="sql_translator.py:can_handle",
        translate_strategy="module_function",
        translate_entrypoint="sql_translator.py:translate",
    )


class SqlTranslatorTests(unittest.TestCase):
    """Verify SQL translation behavior."""
    ...
    def test_can_handle_suffix(self):
        ...
        self.assertTrue(
            module.can_handle(
                path=Path("schema.sql"), declaration=_declaration()
            )
        )
