"""Unit tests for the Go language translator."""
...
_TRANSLATOR_PATH = (
    _REPO_ROOT / "devcovenant/builtin/profiles/go/go_translator.py"
)
...
def _load_translator_module():
    ...
    spec = importlib.util.spec_from_file_location(
        "go_translator", _TRANSLATOR_PATH
    )
...
def _declaration() -> TranslatorDeclaration:
    ...
    return TranslatorDeclaration(
        ...
        can_handle_entrypoint="go_translator.py:can_handle",
        translate_strategy="module_function",
        translate_entrypoint="go_translator.py:translate",
    )


class GoTranslatorTests(unittest.TestCase):
    """Verify Go translation behavior."""
    ...
    def test_can_handle_suffix(self):
        ...
        self.assertTrue(
            module.can_handle(
                path=Path("sample.go"), declaration=_declaration()
            )
        )
