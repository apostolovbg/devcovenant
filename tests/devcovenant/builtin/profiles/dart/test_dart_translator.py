"""Unit tests for the Dart language translator."""
...
_TRANSLATOR_PATH = (
    _REPO_ROOT / "devcovenant/builtin/profiles/dart/dart_translator.py"
)
...
def _load_translator_module():
    ...
    spec = importlib.util.spec_from_file_location(
        "dart_translator", _TRANSLATOR_PATH
    )
...
def _declaration() -> TranslatorDeclaration:
    ...
    return TranslatorDeclaration(
        ...
        can_handle_entrypoint="dart_translator.py:can_handle",
        translate_strategy="module_function",
        translate_entrypoint="dart_translator.py:translate",
    )


class DartTranslatorTests(unittest.TestCase):
    """Verify Dart translation behavior."""
    ...
    def test_can_handle_suffix(self):
        ...
        self.assertTrue(
            module.can_handle(
                path=Path("sample.dart"), declaration=_declaration()
            )
        )
    ...
    def test_translate_emits_identifier_and_risk_facts(self):
        ...
        unit = module.translate(
            path=Path("sample.dart"),
            source='// docs\nvoid run() { Process.run("ls", []); }\n',
