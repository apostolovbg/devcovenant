"""Unit tests for the Ruby language translator."""
...
_TRANSLATOR_PATH = (
    _REPO_ROOT / "devcovenant/builtin/profiles/ruby/ruby_translator.py"
)
...
def _load_translator_module():
    ...
    spec = importlib.util.spec_from_file_location(
        "ruby_translator", _TRANSLATOR_PATH
    )
...
def _declaration() -> TranslatorDeclaration:
    ...
    return TranslatorDeclaration(
        ...
        can_handle_entrypoint="ruby_translator.py:can_handle",
        translate_strategy="module_function",
        translate_entrypoint="ruby_translator.py:translate",
    )


class RubyTranslatorTests(unittest.TestCase):
    """Verify Ruby translation behavior."""
    ...
    def test_can_handle_suffix(self):
        ...
        self.assertTrue(
            module.can_handle(
                path=Path("sample.rb"), declaration=_declaration()
            )
        )
