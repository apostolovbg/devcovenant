"""Unit tests for the TypeScript language translator."""
...
def _load_translator_module():
    ...
    spec = importlib.util.spec_from_file_location(
        "typescript_translator", _TRANSLATOR_PATH
    )
...
def _declaration() -> TranslatorDeclaration:
    ...
    return TranslatorDeclaration(
        translator_id="typescript",
        profile_name="typescript",
        extensions=(".ts",),
        can_handle_strategy="module_function",
        can_handle_entrypoint="typescript_translator.py:can_handle",
        translate_strategy="module_function",
        translate_entrypoint="typescript_translator.py:translate",
    )


class TypescriptTranslatorTests(unittest.TestCase):
    """Verify TypeScript translation behavior."""
    ...
    def test_can_handle_suffix(self):
        ...
        self.assertTrue(
            module.can_handle(
                path=Path("sample.ts"), declaration=_declaration()
            )
        )
