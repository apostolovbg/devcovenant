...
def _unit_test_default_empty_scope_allows_changes(tmp_path: Path):
    """Empty include selectors should disable read-only enforcement."""
    ...
    assert not violations, "Empty selector scope should not block changes"


def _unit_test_blocks_configured_read_only_change(tmp_path: Path):
    """Configured include scope should block in-scope changes."""
    target = _prepare_file(tmp_path)
    ...
    assert violations, "Configured read-only scope must block in-scope edits"
...
class GeneratedUnittestCases(unittest.TestCase):
    ...
    def test_default_empty_scope_allows_changes(self):
        """Run test_default_empty_scope_allows_changes."""
        with tempfile.TemporaryDirectory() as temp_dir:
            tmp_path = Path(temp_dir).resolve()
            _unit_test_default_empty_scope_allows_changes(tmp_path=tmp_path)

    def test_blocks_configured_read_only_change(self):
        """Run test_blocks_configured_read_only_change."""
        with tempfile.TemporaryDirectory() as temp_dir:
            tmp_path = Path(temp_dir).resolve()
            _unit_test_blocks_configured_read_only_change(tmp_path=tmp_path)
