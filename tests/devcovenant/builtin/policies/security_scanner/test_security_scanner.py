...
def _unit_test_reports_utf8_decode_errors(tmp_path: Path):
    """Undecodable files should emit a deterministic policy violation."""
    target = tmp_path / "project_lib" / "binary.py"
    target.parent.mkdir(parents=True, exist_ok=True)
    ...
    assert (
        "Unable to read file as UTF-8 while scanning security constructs"
...
class GeneratedUnittestCases(unittest.TestCase):
    ...
    def test_reports_utf8_decode_errors(self):
        """Run test_reports_utf8_decode_errors."""
        with tempfile.TemporaryDirectory() as temp_dir:
            tmp_path = Path(temp_dir).resolve()
            _unit_test_reports_utf8_decode_errors(tmp_path=tmp_path)
