...
def test_patch_overrides_metadata_options():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        devcov_dir.mkdir()
        (devcov_dir / "common_policy_scripts").mkdir(parents=True)
        ...
        policy_script = (
            devcov_dir / "common_policy_scripts" / "line_length_limit.py"
        )
        policy_script.write_text(
            "from devcovenant.base import PolicyCheck, Violation\n"
        ...
        patch_file = (
            devcov_dir / "common_policy_patches" / "line_length_limit.yml"
        )
        patch_file.write_text("max_length: 100\n", encoding="utf-8")
