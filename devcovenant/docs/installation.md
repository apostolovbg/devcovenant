...
- root CLI (command-line interface) commands emit a standard `Run logs:`
  pointer
...
- `pyproject.toml` now includes the `packaging` runtime dependency so the
  version stack can validate PEP (Python Enhancement Proposal) 440 version
  strings directly, both when
...
- `pyproject.toml` no longer depends on `tqdm`; normal-mode liveness uses
  runtime messages and run-log artifacts instead of progress-bar
...
- when `managed-environment` is active, later CLI commands invoked from a
...
- repository tests compare the tracked
  ...
  against the global asset on critical CI (continuous integration) contract
  fields (job env and DevCovenant gate/test command sequence) to catch drift
  early
