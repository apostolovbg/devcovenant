...
`core_invariants.devflow-run-gates.required_commands` test evidence.
...
- explicit policy-born commands dispatch through declared policy runtime
  actions; dependency management uses the canonical command
...
- persisted run metadata in `run.json` now redacts obvious secret-like values
  from command arguments and structured metadata fields, while raw
  `stdout.log` / `stderr.log` output stays faithful to what child commands
  emitted
...
Artifact privacy rule:
- `run.json` is structured metadata and redacts obvious secret-like command
  arguments and metadata fields before persistence
- `stdout.log`, `stderr.log`, and `tail.txt` are not content-aware secret
  scrubbers; if a child command prints a secret, the printed value remains in
  those logs until the operator removes or cleans them
- `session_snapshot.json` stores path-and-hash style session evidence, not
  full source-file contents
- use `PRIVACY.md` for the full operator-facing data-handling explanation
...
- runs the full start -> test -> end sequence on Python `3.14`
- runs a focused compatibility matrix on Python `3.10` through `3.13`
  so the supported-version claim is backed by explicit CI evidence instead of
  a loose `3.x` check
- runs an assurance job that executes `pip-audit -r requirements.lock` and
...
- `build.yml`:
  ...
  pre-build artifact cleanup, CycloneDX SBOM generation, and isolated wheel +
  sdist install smoke (`python -m devcovenant --help`)
- `publish.yml`:
  manual release workflow (`workflow_dispatch`) with the same cleanup, SBOM
  generation, and wheel + sdist install smoke in its build job before
  trusted publishing to PyPI
- `.github/dependabot.yml`:
  weekly dependency-review automation for `github-actions` and Python package
  metadata at the repository root

Trusted publishing note:
- the publish workflow assumes the repository has already been registered as a
  trusted publisher on the PyPI project side
- once that PyPI-side trust is configured, the publish action no longer needs
  a long-lived upload token
- package attestations are emitted as part of the PyPI trusted-publishing
  path rather than as an unrelated side workflow
