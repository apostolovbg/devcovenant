...
_DEFAULT_STATUS = Path(".devcov-state") / "test_status.json"
...
def _git_changed_files(repo_root: Path) -> list[Path]:
    """Return changed files from git status, if available."""
    try:
    ...
    for line in result.stdout.splitlines():
        ...
        paths.append(repo_root / path_part)
    return paths
...
def _ensure_status_file(status_file: Path) -> bool:
    """Create the status file if missing; return True when created."""
...
class DevflowRunGates(PolicyCheck):
    ...
    def check(self, ctx: CheckContext) -> List[Violation]:
        ...
        if ctx.mode == "pre-commit":
            git_files = _git_changed_files(repo_root)
            if git_files:
                files = git_files
        ...
        if code_mtime == 0.0:
            return violations
        earliest_code_mtime = _earliest_code_mtime(files, extensions)
        ...
        _ensure_status_file(status_path)
        status = _load_test_status(status_path)
        if not status:
            violations.append(
                Violation(
                    ...
                    message=(
                        "Code changed but "
                        ".devcov-state/test_status.json is missing or empty; "
                ...
                violations.append(
                    Violation(
                        ...
                        message=(
                            ...
                            "`python3 devcovenant/run_pre_commit.py "
                            "--phase start` before editing code."
                        ),
                    )
                )
                ...
                violations.append(
                    Violation(
                        ...
                        message=(
                            ...
                            "changes. Run `python3 devcovenant/"
                            "run_pre_commit.py --phase end` after edits "
                            "complete."
                        ),
                    )
                )
