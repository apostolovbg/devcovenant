"""C# adapter for modules-need-tests."""
...
def _is_module(
    ...
    """Return True when the path is a C# source under enforcement."""
    if path.suffix.lower() != SUFFIX:
        return False
    if path.name.lower().endswith(("test.cs", "tests.cs")):
        return False
...
def _test_exists(repo_root: Path, tests_dirs: List[str], stem: str) -> bool:
    """Return True if *Test.cs/Tests.cs exists under tests dirs."""
    needle = stem.lower()
    for tests_dir in tests_dirs:
        ...
        for candidate in root.rglob("*.cs"):
            ...
            if not (name.endswith("test.cs") or name.endswith("tests.cs")):
                continue
            if needle in name:
                return True
    return False
...
def check_changes(
    *,
    ...
    """Return violations for C# modules lacking tests."""
        ...
        if not has_test:
            violations.append(
                Violation(
                    ...
                    severity="error",
                    file_path=module,
                    message="Add *Test.cs for changed C# module.",
                )
            )
