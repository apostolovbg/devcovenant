"""JS/JSX adapter for new-modules-need-tests."""
...
def _tests_touched(repo_root: Path, tests_dirs: List[str], paths: Set[Path]):
    ...
    for path in paths:
        try:
            ...
            continue
        if any(
            rel == tdir or rel.startswith(f"{tdir}/") for tdir in tests_dirs
        ):
            return True
    return False
...
def check_changes(
    *,
    ...
    """Return violations for new JS modules without tests."""
        ...
        if not _test_exists(repo_root, tests_dirs, stem) and not changed_tests:
            violations.append(
                Violation(
                    ...
                    file_path=module,
                    message=(
                        f"Add a *.test.js/.spec.js (or __tests__) for {stem}"
                    ),
                )
            )

    if removed_modules and not changed_tests:
        first = removed_modules[0]
        violations.append(
            Violation(
                policy_id=policy_id,
                severity="error",
                file_path=first,
                message="Update tests to reflect removed JS modules.",
            )
        )
