"""Java adapter for doc/comment coverage."""
...
METHOD_PATTERN = re.compile(
    r"\b(public|protected|private)\s+[\w<>\[\]]+\s+"
...
def check_file(path: Path, source: str, policy_id: str) -> List[Violation]:
    """Return violations for Java classes/methods missing doc comments."""
    ...
    def record(name: str, lineno: int):
        ...
        violations.append(
            Violation(
                ...
                file_path=path,
                line_number=lineno,
                message=f"{name} is missing a Javadoc or comment.",
            )
        )
