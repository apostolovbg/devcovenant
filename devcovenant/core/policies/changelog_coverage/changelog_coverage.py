...
_ALLOWLIST_DOC_SUFFIXES = {".md", ".rst", ".txt"}
_ALLOWLIST_HEADER_KEYS = {"last updated", "version"}
_ALLOWLIST_HEADER_SCAN_LINES = 4
_ALLOWLIST_MANAGED_MARKERS = (
...
def _marker_signature(content: str) -> str:
    """Return a deterministic marker-sequence signature."""
    tokens: list[str] = []
...
def _managed_ranges(content: str) -> list[tuple[int, int]]:
    ...
    for begin_marker, end_marker in _ALLOWLIST_MANAGED_MARKERS:
...
def _allowlist_fingerprint_for_path(
    repo_root: Path,
    relative_path: str,
    *,
    header_doc_suffixes: set[str],
    ...
    """Return current allowlist fingerprint entry for one repository path."""
...
def _normalize_exemption_entry(
    raw_entry: object,
    *,
    relative_path: str,
) -> dict[str, str]:
    """Validate one allowlist fingerprint entry from gate status."""
...
_COLLECTIONS_DISABLE_TOKENS = {"__none__", "none", "off", "false", "no"}
