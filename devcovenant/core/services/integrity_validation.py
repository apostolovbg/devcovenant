"""Core invariant: DevCovenant descriptor, registry, and gate integrity."""
...
def _relative_path_option(
    ...
    """Return one repo-relative path from string-or-list metadata."""
    value = invariant.get_option(key, default)
...
class IntegrityValidationInvariant(CoreInvariantCheck):
            ...
            return [
                Violation(
                    ...
                    message=(
                        ...
                        "Run `devcovenant run` so the workflow runs "
                        "execute and the status file is refreshed."
                    ),
                )
            ]
    ...
    def check(self, context: CheckContext) -> List[Violation]:
        ...
        violations.extend(self._check_gate_status(context, status_relative))
        return violations
